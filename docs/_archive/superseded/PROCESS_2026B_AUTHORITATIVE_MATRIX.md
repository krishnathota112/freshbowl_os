...
> ## ⚠ NOT THE RUNNING SYSTEM — read this first
>
> **29 August 2026.** This matrix describes a model the code **does not currently implement**.
>
> It says H0 is Bagasse Wetting (`FIB1-HOP-1` H0→H3, `LAB-FIB-WET` H1→H2) with weighment outside the
> rail as a pre-H0 zone. The seed files — `mushroomos/supabase/seed/s03`, `s08` — place those
> activities on **Day 1**, and the test suite asserts Day 1. The seeds were never updated to match
> this document.
>
> The live database *did* match this matrix. It was restored to the seed definition on 29 Aug 2026 so
> that the tests, the build and the web demo have one definition to agree on. That was a working
> decision, **not a ruling that this matrix is wrong.**
>
> **Where H0 starts is an open question for the process owner.** The divergent model is preserved
> verbatim and can be reinstated. Do not treat either version as settled factory truth.
>
