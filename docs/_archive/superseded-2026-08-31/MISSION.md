...
git status          # review every move before committing
git commit -m "docs: restructure — 11 live documents, everything else archived"
```

The script is idempotent and deletes nothing. It moves files with `git mv`, so history follows them.
It prints its own proof at the end.

**Acceptance — the script asserts all three:**

1. The repository root holds `CLAUDE.md` and `README.md` and no other markdown.
2. Eleven live documents in six numbered folders under `docs/`, never more than three per folder.
3. Every `552` outside `_archive/` and `_reference/` is historical context — "the old model",
   "sweep this out", "this is why" — and never an instruction.

**Do not** rewrite the archived documents. They are the record of how the project got here, and
several contain reasoning worth keeping. They are wrong only as instructions, which is why they move
rather than being edited.

**Then, and only then**, `docs/_reference/` still needs an H552 sweep — about twenty files kept
because a live test or migration points at them, not because they are current. That is a Phase 2
...
`docs/04-audit/WRITE-PATH-MATRIX.md` already contains eight rows filled in from the migration files.
...
- `docs/04-audit/WRITE-PATH-MATRIX.md` — every fact, every write path, the full template per row.
- `docs/04-audit/SECURITY-ATTACK-MATRIX.md` — each attempted forgery, the role used, the exact call,
  and what the database did.
- `docs/04-audit/FOUNDATION-STATUS.md` — GREEN / AMBER / RED / UNRESOLVED per freeze §24 heading.
...
**Acceptance:** `docs/04-audit/REBUILDABILITY.md` records a clean rebuild that was actually performed,
...
Freeze §19 in full. `docs/02-architecture/DATA-CONTRACTS.md`, one section per screen, with
...
Freeze §20. `docs/05-ui/UI-SYSTEM.md`, `COMPONENT_INVENTORY.md`, `SCREEN_CONTRACT_INDEX.md`,
`DESIGN_DECISIONS.md`.

**Start from `docs/05-ui/VISUAL-LANGUAGE.md`**, which reads the stakeholder HTML prototype and says
...
| **Person C** | The **component library**, built against fixtures. Take the ten primitives from `docs/05-ui/VISUAL-LANGUAGE.md` and the field-level payloads in section 9 of the Technical PRD. Build each with a typed props interface and a fixture file — no network calls. | A component that accepts a typed payload does not care whether the payload is real yet. This is the largest body of screen work and it can be done before the backend is green, provided **nothing computes** — no variance, no eligibility, no status, no "on time". |
