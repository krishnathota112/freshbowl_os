-- ─────────────────────────────────────────────────────────────────────────────
-- 0098 · A DEMO / TEST BATCH HAS NO CLOCK (user, 15 Sep 2026).
--   "Demo Batch should be a testing mode, not a time-restricted batch … The timeline and sequencing
--    rules apply to real batches."
-- For a batch with is_demo = true (set by mark_batch_demo, admin, draft only, one way, audited):
--   · time gates (MIN_DURATION): the time part is met at once — after photo and Finish are not held;
--   · rests between steps (min_rest_hr), ELAPSED_TIME and DAY0_DURATION rests: met at once.
-- Unchanged for demo: step order, Start before Finish, photos, readings and checklists, temperature
-- triggers, Lab checks locked to their field step, Lab approvals, role boundaries. Real batches: unchanged.
-- Generated by build_0098.py from the live function bodies. Rollback: run pre_0098__functions.sql.
-- ─────────────────────────────────────────────────────────────────────────────

create or replace function public.batch_clock_waived(p_batch uuid)
returns boolean language sql stable security definer set search_path = public
as $fn$ select coalesce((select mb.is_demo from master_batch mb where mb.id = p_batch), false) $fn$;
revoke all on function public.batch_clock_waived(uuid) from public;
grant execute on function public.batch_clock_waived(uuid) to authenticated;

CREATE OR REPLACE FUNCTION public.time_gate_status(p_activity uuid)
 RETURNS TABLE(has_gate boolean, hours numeric, measured_from timestamp with time zone, ready_at timestamp with time zone, time_ok boolean, ready_label text, remaining_label text, trigger_label text, trigger_min numeric, trigger_join text, trigger_value numeric, trigger_ok boolean, ok boolean, summary text)
 LANGUAGE plpgsql
...
begin
  ...
  -- 0098 · a DEMO / TEST batch has no clock: the time part is met at once (triggers and order still apply)
  if public.batch_clock_waived(ba.master_batch_id) then
    v_time := true; v_ready := null; v_rdy := null; v_rem := null;
  end if;
