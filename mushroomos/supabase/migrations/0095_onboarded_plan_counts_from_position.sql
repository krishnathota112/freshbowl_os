-- ─────────────────────────────────────────────────────────────────────────────
-- 0095 · An onboarded running batch is planned from its current position, at the moment of onboarding.
--
-- User, 15 Sep 2026: "the system time is counted from the point of process — anywhere in the process the
-- time is counted from there". A new batch plans from H0 (unchanged). A running batch plans from each
-- selected position = now; later steps follow at the process's hour offsets; where streams meet, the later
-- stream governs; not-started feeder streams are planned to be ready when needed; Lab checks follow their
-- field step; before-tracking work has no planned time. H0, if given, is still recorded but does not move
-- the plan. Versions without planned hours plan nothing (as before).
-- ─────────────────────────────────────────────────────────────────────────────
...
begin
  ...
  delete from pg_temp.ob_plan;
  delete from pg_temp.ob_edge;
...
-- 0095 · THE LAB SEES ITS OWN WORK IN v_my_work. The view showed a lab technician only work assigned to them
-- by name; nothing is assigned by name (role-level work), so the Lab queue had no planned time, photo counts
-- or lock reasons for any checkpoint. A lab technician now sees every Lab checkpoint (responsible_role
-- lab_tech) of an active batch — the same role-level rule the Supervisor already has. Columns unchanged.
do $$
declare d text; old_w text := '((ba.assigned_person_id = auth.uid()) OR (current_app_role() = ANY (ARRAY[''supervisor''::app_role, ''manager''::app_role, ''admin''::app_role, ''gm''::app_role])))';
begin
  d := pg_get_viewdef('public.v_my_work'::regclass);
  if (length(d) - length(replace(d, old_w, ''))) / length(old_w) <> 1 then
    raise exception '0095: v_my_work visibility clause not found exactly once';
  end if;
  d := replace(d, old_w, '((ba.assigned_person_id = auth.uid()) OR (current_app_role() = ANY (ARRAY[''supervisor''::app_role, ''manager''::app_role, ''admin''::app_role, ''gm''::app_role])) OR ((current_app_role() = ''lab_tech''::app_role) AND (ba.responsible_role = ''lab_tech''::app_role)))');
  execute 'create or replace view public.v_my_work with (security_invoker = on) as ' || d;
end $$;
