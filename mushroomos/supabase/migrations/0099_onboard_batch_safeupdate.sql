-- 0099 · ONBOARDING FAILED FROM THE APP: "DELETE requires a WHERE clause".
-- Supabase loads the safeupdate extension for API requests, which refuses a DELETE with no WHERE. 0095's
-- onboard_batch clears its two scratch tables with a bare DELETE, so every onboarding from the Admin screen
-- failed (direct database tests do not load safeupdate, so they passed). Only those two statements change.
-- Rollback: db-rollback-2026-09-13/pre_0099__onboard_batch.sql
