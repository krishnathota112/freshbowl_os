-- ─────────────────────────────────────────────────────────────────────────────
-- 0126 · A REJECTED LAB SUBMISSION GOES BACK TO THE LAB, AND ITS RE-SUBMISSION REACHES THE GM (16 Sep 2026)
--
-- Found by scripts/demo-lab-gate-proof.mjs (owner: demo batches must show Lab → GM → remark → Supervisor).
-- After a GM rejection the Lab activity stayed COMPLETED, the Lab screen offered no re-test (it only
-- re-tests open work) and v_lab_approval_queue.awaiting_decision was true only when NO decision existed,
-- so a re-submission would never reach the GM. The gated production step could never open. Real and demo alike.
--
--   decide_lab_submission   'rejected' → the Lab activity becomes RETURNED with the remark; Lab notified.
--   v_lab_approval_queue    awaiting_decision also when the latest verdict is 'rejected' and the work is COMPLETED
--                           again — since a rejection now returns it, COMPLETED after a rejection means re-submitted.
-- Nothing else: gate evaluation, approval roles and the approve path are unchanged.
-- Rollback: db-rollback/pre_0126__lab_rejection_returns.sql
-- ─────────────────────────────────────────────────────────────────────────────
...
begin
   ...
   */
  -- 0126 · A REJECTION RETURNS THE WORK TO THE LAB.
  -- Before 0126 a rejected submission stayed COMPLETED: the Lab screen could not re-test it and the approval
  -- queue never showed it again, so a rejected gate could never be opened. Now it goes back to the Lab as
  -- RETURNED with the approver's remark — the state the Lab screen already handles ("Returned after a
  -- rejection") — and the Lab is told. The gate stays shut: its latest decision is the rejection until a
  -- new submission is approved. An actor transition (a verdict), like return_activity.
  if p_verdict = 'rejected' then
    update batch_activity
       set state = 'RETURNED',
           blocked_reason = 'Rejected by ' || coalesce(role_::text, 'approver') || ' — ' || p_reason
     where id = p_activity;
    insert into notification (master_batch_id, batch_activity_id, to_role, kind, message, reason, sent_by)
    values (ba.master_batch_id, ba.id, 'lab_tech', 'returned',
            ba.title || ' — ' || coalesce(ba.scope_label, 'Whole batch') || ' was rejected. Re-test and submit again.',
            p_reason, auth.uid());
  end if;
 ...
 SELECT master_batch_id,
   ...
   FROM ( WITH latest AS (
         ...
         SELECT ba.master_batch_id,
            ...
            ((ba.state = 'COMPLETED'::activity_state) AND ((l.verdict IS NULL) OR (l.verdict = 'rejected'::text))) AS awaiting_decision,
