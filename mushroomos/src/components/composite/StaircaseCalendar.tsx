/**
 * THE BOARD — many batches, one calendar. `UI_CONTROL_TOWER_SPEC §9.1`.
 *
 * Each batch is the same length and each starts a fixed interval after the last, so drawn on a shared
 * calendar axis they form a diagonal staircase. That one image teaches the business model in two
 * seconds: twenty-three days each, a new one every two days, twelve alive at once.
 *
 * L3 — takes data as props. No query, no `api/`.
 *
 * THE THREE RULES THIS COMPONENT EXISTS TO KEEP
 *   B.4 · bars are sorted by `startAt` and NOTHING re-sorts them. §9.3 lists "sorting that destroys
 *         the diagonal" as a thing the board must never do; exceptions go in the band above.
 *   B.5 · exactly ONE `now` line down the whole board. Not one per row. It is rendered once, outside
 *         the row loop, from a single number — the geometry returns a value, not a list.
 *   B.7 · at 375 px the staircase is a vertical list and the PAGE does not scroll horizontally. The
 *         wide form lives inside its own `overflow-x-auto` container (cross-cutting criterion 87).
 */
...
export function StaircaseCalendar({
  ...
  nowMs: number;
  /** §9.2 — clicking a bar opens that batch AT THAT HOUR, not its home page. */
  ...
  return (
    <div className="flex flex-col gap-2">
      {/*
        WIDE FORM · the diagonal. Scrolls inside ITS OWN container, so the page body never scrolls
        horizontally at any breakpoint — criterion 87 and B.7.
      */}
      <div className="hidden overflow-x-auto md:block">
        <div className="relative min-w-[640px] pt-6">
          {/* The calendar axis: dates and weekday names, never batch-day indices — §9.2. */}
          ...
          {/*
            THE ONE `now` LINE. Rendered once, here, spanning every row — not once per bar.
            §9.2: "One now line down the whole board. Not twelve markers."
          */}
          <div
                    ...
                    >
                      {placement.segments.map((s, i) => (
                        <button
                          ...
                          type="button"
                          // Clicking lands on the hour under the click, not on the batch's home page.
      ...
      {/*
        NARROW FORM · a vertical list. B.7 — at 375 px the staircase is a list, and there is nothing
        to scroll sideways. The same order, the same data; a diagonal is not readable at that width and
        pretending otherwise is what causes the horizontal scroll the criterion forbids.
      */}
...
/** The five meanings, named. State is never carried by colour alone — criterion 81. */
function Legend() {
  return (
