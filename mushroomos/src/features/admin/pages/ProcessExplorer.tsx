...
import { FiveLayerNode } from '../../../shared/ui/graph/FiveLayerNode';
import { PageHeading } from '../../../shared/ui/layout/PageHeading';
import { Card, Chip, ConflictMarker, EmptyState, NumberInput, Stat } from '../../../shared/ui/primitives';
      ...
      <div className="grid gap-4 lg:grid-cols-[320px_1fr]">
        <aside className="flex flex-col gap-4">
          ...
          <LoadPlanPanel
            ...
            roleLead={roleLead}
            onChange={(patch: Partial<BatchConfigView>) => setConfig((c) => ({ ...c, ...patch }))}
          />
          <DependencyPanel config={config} />
          <StructurePanel config={config} onChange={(patch: Partial<BatchConfigView>) => setConfig((c) => ({ ...c, ...patch }))} />
        </aside>
      ...
      <div className="flex flex-col gap-2">
        <label className="flex items-center justify-between gap-2">
          ...
          <NumberInput
            ...
            step={0.5}
            unit="MT"
            onChange={(n: number) => onChange({ primary_fibre_required_mt: n })}
          />
        </label>
        <label className="flex items-center justify-between gap-2">
          ...
          <NumberInput
            ...
            step={0.5}
            unit="MT"
            onChange={(n: number) => onChange({ expected_load_capacity_mt: n })}
          />
        </label>
      </div>
      ...
      {result.ok ? (
        <>
          ...
          <p className="mt-2 mono text-[11px] leading-relaxed text-muted">
            ...
            {result.instances
              .slice(0, 4)
              .map((i: { plannedQuantityMt?: number }) => `${i.plannedQuantityMt}`)
              .join(' · ')}
        ...
        {fields.map((f) => (
          <label key={f.key} className="flex items-center justify-between gap-2">
            ...
            <NumberInput
              ...
              step={1}
              onChange={(n: number) => onChange({ [f.key]: n })}
            />
          </label>
        ))}
      </div>
                ...
                layers={{
                  ...
                  plan: card.ok
                    ? {
                        text: card.instances
                          .slice(0, 3)
                          .map((i: { scopeLabel: string }) => i.scopeLabel)
