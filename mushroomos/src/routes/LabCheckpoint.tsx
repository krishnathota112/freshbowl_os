...
import { EvidenceStep, requirementsFrom } from '../components/field/EvidenceStep';
import { ErrorPanel } from '../components/field/ErrorPanel';
import { approverWords, fmtTime, fmtWhen, labStatus, paramLabel } from '../components/field/labWords';
import { Chip, Skeleton } from '../components/primitives';

/**
 * L2 · one checkpoint, start to submit. `WORKSTATIONS.md` §3, task `UI-001`.
 *
 *     1 Sample   →   2 Readings   →   3 Photos   →   4 Submit   →   Waiting approval
 *
 * THE URL IS THE STATE. `/lab/checkpoint/:activityId`, and everything on the screen is re-read from
 * the server — so a refresh, an app restart or a dropped connection lands on exactly the same step,
 * with nothing held only in the phone.
 *
 * WHAT THE SERVER DECIDES, and this screen only shows:
 *   · whether the work may start          `start_activity` refuses with a sentence
 *   · which checkpoint a sample may use    `open_lab_sample` refuses any other
 *   · whether a reading passes             `record_lab_result` returns the verdict
 *   · whether the photographs are enough   `complete_activity` returns what is outstanding
 *   · whether the gate opens               only `decide_lab_submission`, by someone else
 *
 * A lab technician never decides a lab submission, including their own. After Submit this screen
 * says who does, and what the decision holds shut.
 */
...
export function LabCheckpoint() {
  ...
  const ctx = useQuery({
    ...
    // While a decision is awaited, look again every 30 s so "Approved" appears without a refresh.
...
function Checkpoint({
  ...
  // Exactly one bound checkpoint is the normal case (all 41 in PROCESS-2026C). Otherwise the
  // technician chooses — the server refuses a checkpoint the activity is not bound to either way.
  ...
  return (
    <>
      ...
      {/* ── What happens next — the decision and the gate ─────────────────────────────── */}
      ...
      {open && sample && (
        <Step n={4} title="Submit" done={false}>
          ...
          {ready ? (
            ...
            // Still tappable: the server decides what is missing, and says so.
            <SecondaryButton
              onClick={() => submit.mutate()}
              disabled={submit.isPending}
...
/** What the decision is, who makes it, and what it holds shut — said in words. */
...
function RecordedReading({ r, canRetest, onDone }: { r: LabResultRow; canRetest: boolean; onDone: () => void }) {
  ...
  // The verdict is the server's, frozen against the range at the moment the test was requested.
  const verdict =
    r.verdict === 'pass'
