 ...
 *
 * `FiveLayerNode` already consumes `Density`; this removes the last reason for a component to know
 * what a role is.
 */
 ...
 *
 * Falls back to `operator` when the role is not yet known — the narrowest view, so a component
 * cannot show management-level detail during the moment before the claim arrives.
 */
