 ...
 * check of the same five demo accounts.
 *
...
const LOCAL_ACCOUNTS = [
  ...
  // Password for this one is `lab123`, NOT the shared demo password the other
  // four use — hence the per-button password on the sign-in screen.
  { email: 'lab@freshbowl.dev',   name: 'Priya',    role: 'lab', h: 'e7d68bbf28cb804eea723e65391852c0032fbe32b0afa60aaebabff045b6e1e3' },
];
...
const ROLE_LABEL = {
  ...
  lab: 'Lab Technician',
  // Kept as an alias: the Supabase path reads role from profiles.role, and that
  // column may still hold the longer spelling for a seeded lab user.
  lab_technician: 'Lab Technician',
};

// Where a role lands after sign-in. A lab tech has no use for the operator home
// screen, and index.html re-routes a stored session through here too, so
// reopening the app does not drop them back onto the wrong home.
const ROLE_LANDING = {
  lab: 'lab_dashboard.html',
};

function landingFor(role) {
  return ROLE_LANDING[role] || 'home.html';
}
...
window.Auth = { signIn, signOut, currentUser, requireAuth, requireRole, renderIdentity, landingFor, ROLE_LABEL };
